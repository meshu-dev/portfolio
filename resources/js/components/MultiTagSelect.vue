...
const props = defineProps({ items: Object, name: String, placeholder: String })
const frameworks = props.items

const frameworksRef = defineModel()

const searchTerm = ref('')
//const frameworksRef = ref(['Nuxt'])
  ...
  <Popover v-model:open="open">
    <ListboxRoot
      ...
      <PopoverAnchor class="inline-flex w-full">
        <TagsInput v-slot="{ modelValue: tags }" v-model="frameworksRef" class="w-full" :name="name">
          <TagsInputItem v-for="item in tags" :key="item.toString()" :value="getItemLabel(item.toString())">
            <TagsInputItemText /><!-- {{ getItemLabel(item.toString()) }} -->
            <TagsInputItemDelete />
          </TagsInputItem>
